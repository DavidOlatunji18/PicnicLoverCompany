...
function ownerEmailHtml(booking: Record<string, unknown>) {
  return `
...
function customerEmailHtml(booking: Record<string, unknown>) {
  return `
    <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; padding: 32px; background: #FAF7F4; border-radius: 12px;">
...
export async function sendBookingEmails(booking: Record<string, unknown>) {
  ...
  const customerCommand = new SendEmailCommand({
    Source: sender,
    Destination: { ToAddresses: [booking.email as string] },
    Message: {
